...
{
    private AutoUpdaterInfo AutoUpdaterInfo;
    private UpdateHelper updateHelper = new UpdateHelper();
    private UpdateStepOn m_UpdateStepOn;
    private UpdateType internalUpdateType = UpdateType.Automatic;
    private UpdateType m_UpdateType = UpdateType.Automatic;


    // changes
    private string version, changes;
    private bool changesAreRTF;
    ...
    private void SetLastSuccessfulStep()
    {
        if (UpdateStepOn == UpdateStepOn.Checking)
    ...
    private void updateHelper_UpdateStepMismatch(object sender, Response respType, UpdateStep previousStep)
    {
    ...
    private void updateHelper_PipeServerDisconnected(object sender, UpdateHelperData e)
    {
    ...
    private void updateHelper_ProgressChanged(object sender, UpdateHelperData e)
    {
    ...
    private void updateHelper_ResendRestartInfo(object sender, EventArgs e)
    {
        // send the client the arguments that need to run on success and failure
        if (ServiceName != null)
            updateHelper.RestartInfo(ServiceName, AutoUpdaterInfo.AutoUpdateID, Arguments, true);
    ...
    private void StartNextStep(UpdateStep updateStepOn)
    {
        // begin the next step
        switch (updateStepOn)
        {
    ...
    private void UpdateReadyToExtract()
    {
        UpdateStepOn = UpdateStepOn.UpdateDownloaded;
    ...
    private void UpdateReadyToInstall()
    {
    ...
    private void UpdateStepFailed(UpdateStepOn us, FailArgs args)
    {
        SetLastSuccessfulStep();
