...
public class RoleGame : Game
{
    ...
    {
        int maxHealth = 0;
        if (p.Role == Role.Ruler && CurrentGame.Players.Count > 4) maxHealth += 1;
        if (CurrentGame.Settings.DualHeroMode && p.Hero2 != null)
        {
    ...
    public class PlayerDealStageTrigger : Trigger
    {
        public override void Run(GameEvent gameEvent, GameEventArgs eventArgs)
        {
            ...
            CurrentGame.DrawCards(currentPlayer, 2 + currentPlayer[Player.DealAdjustment]);
        }
    }
    ...
    public class PlayerDiscardStageTrigger : Trigger
    {
        public override void Run(GameEvent gameEvent, GameEventArgs eventArgs)
        {
            ...
            CurrentGame.ForcePlayerDiscard(currentPlayer,
                (p, d) =>
                {
                    int i = CurrentGame.Decks[p, DeckType.Hand].Count - Math.Max(0, p.Health) - args.AdjustmentAmount;
                    if (i < 0) i = 0;
                    return i;
                },
                false,
    ...
    public class CommitActionToTargetsTrigger : Trigger
    {
        public override void Run(GameEvent gameEvent, GameEventArgs eventArgs)
        {
                ...
                || CardCategoryManager.IsCardCategory(c.Type.Category, CardCategory.Equipment))
            {
                ...
                CurrentGame.Emit(GameEvent.PlayerUsedCard, newArg);
                if (c.Type.IsCardCategory(CardCategory.DelayedTool))
                {
                    CurrentGame.Emit(GameEvent.CardUsageTargetConfirming, newArg);
                    CurrentGame.Emit(GameEvent.CardUsageTargetConfirmed, newArg);
                }
            ...
            CurrentGame.MoveCards(m, false, GameDelays.PlayerAction);
            if (isDoingAFavor != eventArgs.Source)
            {
                CurrentGame.PlayerPlayedCard(isDoingAFavor, new List<Player>() { eventArgs.Source }, c);
                CurrentGame.PlayerLostCard(isDoingAFavor, eventArgs.Cards != null ? eventArgs.Cards : ((CompositeCard)c).Subcards);
            }
            else
            {
                CurrentGame.PlayerLostCard(eventArgs.Source, eventArgs.Cards);
            }
            ...
            CurrentGame.SortByOrderOfComputation(CurrentGame.CurrentPlayer, arg.Targets);
            if (runTrigger)
            {
                try
                {
                    CurrentGame.Emit(GameEvent.PlayerUsedCard, arg);
                    CurrentGame.Emit(GameEvent.CardUsageTargetConfirming, arg);
                    CurrentGame.Emit(GameEvent.CardUsageTargetConfirmed, arg);
                }
                catch (TriggerResultException)
                {
            ...
            if (runTrigger)
            {
                try
                {
                    CurrentGame.Emit(GameEvent.CardUsageDone, arg);
                }
                catch (TriggerResultException)
                {
                    throw new NotImplementedException();
            ...
            if (CurrentGame.Decks[DeckType.Compute].Count > 0)
            {
                m.Cards = new List<Card>(CurrentGame.Decks[DeckType.Compute]);
                m.To = new DeckPlace(null, DeckType.Discard);
                m.Helper = new MovementHelper();
                CurrentGame.PlayerAboutToDiscardCard(savedSource, m.Cards, DiscardReason.Use);
                CurrentGame.MoveCards(m, false, GameDelays.None);
                CurrentGame.PlayerDiscardedCard(savedSource, m.Cards, DiscardReason.Use);
            }
            Trace.Assert(CurrentGame.Decks[DeckType.Compute].Count == 0);
            CurrentGame.Decks[DeckType.Compute].AddRange(computeBackup);
        }
    }
    ...
    private class RoleGameRuleTrigger : Trigger
    {
        ...
        public override void Run(GameEvent gameEvent, GameEventArgs eventArgs)
        {
            RoleGame game = CurrentGame as RoleGame;
            ...
            foreach (var p in toCheck)
            {
                ...
                {
                    if (heroIndex == 1 && !CurrentGame.Settings.DualHeroMode) break;
    ...
    public class DoPlayerTrigger : Trigger
    {
        public override void Run(GameEvent gameEvent, GameEventArgs eventArgs)
        {
            ...
            CurrentGame.Emit(GameEvent.PhaseBeforeStart, new GameEventArgs() { Source = currentPlayer });
            while (true)
            {
                if (game.PhasesSkipped.Contains(game.CurrentPhase) && game.CurrentPhaseEventIndex < PhaseEvents.Length - 1)
                {
                ...
                if (game.CurrentPhaseEventIndex >= PhaseEvents.Length || currentPlayer.IsDead)
                {
                    game.CurrentPhaseEventIndex = 0;
                    game.CurrentPhase++;
            ...
            CurrentGame.Emit(GameEvent.PhasePostEnd, new GameEventArgs() { Source = currentPlayer });
    ...
    {
        private readonly DeckType role = DeckType.Register("Role");
        private void RevealAllPlayersRoles()
        {
            foreach (var player in CurrentGame.Players)
            {
                CurrentGame.SyncImmutableCardAll(CurrentGame.Decks[player, role][0]);
                var r = (CurrentGame.Decks[player, role][0].Type as RoleCardHandler).Role;
                if (player.Role != r)
                {
                    player.Role = r;
                }
        ...
        public override void Run(GameEvent gameEvent, GameEventArgs eventArgs)
        {
            ...
            CurrentGame.Emit(GameEvent.BeforeRevealRole, eventArgs);

            Trace.Assert(CurrentGame.Decks[p, role].Count == 1);
            ...
            if (p.Role == Role.Ruler)
            {
                ...
                Role winRole;
                if (CurrentGame.AlivePlayers.Count == 2 && deadRebel == (CurrentGame as RoleGame).NumberOfRebels)
                {
            ...
            else
            {
                ...
                Trace.TraceInformation("Deathtoll: Rebel {0}/{1}, Defector {2}/{3}", deadRebel, (CurrentGame as RoleGame).NumberOfRebels, deadDefector, (CurrentGame as RoleGame).NumberOfDefectors);
                if (deadRebel == (CurrentGame as RoleGame).NumberOfRebels && deadDefector == (CurrentGame as RoleGame).NumberOfDefectors)
                {
                    Trace.TraceInformation("Ruler wins.");
                    RevealAllPlayersRoles();
                    var winners = from pl in CurrentGame.Players where pl.Role == Role.Ruler || pl.Role == Role.Loyalist select pl;
                    p.IsDead = true;
                    throw new GameOverException(false, winners);
                }
            }
            ...
            if (p.Role == Role.Rebel || p.Role == Role.Defector)
            {
                if (source != null && !source.IsDead && p.Role == Role.Rebel)
                {
                    ...
                    CurrentGame.DrawCards(source, 3);
                }
            }
            if (p.Role == Role.Loyalist && source != null && source.Role == Role.Ruler)
            {
                ...
                CurrentGame.MoveCards(move);
                CurrentGame.PlayerLostCard(p, cards);
            }
        }
    }
    ...
    protected class ShuffleTrigger : Trigger
    {
        public override void Run(GameEvent gameEvent, GameEventArgs eventArgs)
        {
            ...
            foreach (var c in CurrentGame.Decks[null, DeckType.Discard])
            {
                ...
                CurrentGame.Decks[null, DeckType.Dealing].Add(c);
            }
            CurrentGame.Decks[null, DeckType.Discard].Clear();
        }
    }
