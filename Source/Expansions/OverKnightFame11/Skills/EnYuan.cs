...
public class EnYuan : TriggerSkill
{
    ...
    private bool CardOriginalOwnerCheck(Card card)
    {
    ...
    private bool EnVerifier(Player Owner, GameEvent gameEvent, GameEventArgs eventArgs)
    {
        enSources.Clear();
    ...
    public void En(Player Owner, GameEvent gameEvent, GameEventArgs eventArgs)
    {
        ...
        {
            int answer = 0;
            Owner.AskForMultipleChoice(new MultipleChoicePrompt("EnYuan", source), Prompt.YesNoChoices, out answer);
            if (answer == 1)
            {
                EnYuanEffect = 0;
                NotifySkillUse();
    ...
    private readonly List<Player> enSources;
