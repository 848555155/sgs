...
public class BasicExpansion : Expansion
{
    public BasicExpansion()
    {
        ...
        TriggerRegistration =
        [
            new DelayedTriggerRegistration() { key = GameEvent.PlayerUsedCard, trigger = new WuGuFengDengTrigger() },
            new DelayedTriggerRegistration() { key = GameEvent.CardUsageBeforeEffected, trigger = new WuXieKeJiTrigger() },
            new DelayedTriggerRegistration() { key = GameEvent.CardUsageBeforeEffected, trigger = new ShaCancelling() },
            new DelayedTriggerRegistration() { key = GameEvent.PlayerDying, trigger = new PlayerDying() { Priority = int.MinValue } },
        ];
    }
}
