...
public class GuanXing : TriggerSkill
{
    ...
    {
        return source.Hero.Name == "JiangWei"
            || source.Hero2 != null
            && source.Hero2.Name == "JiangWei" ? 1 : 0;
    }

    private class GuanXingVerifier(List<Card> c) : ICardChoiceVerifier
    {
        ...
        private readonly List<Card> cards = new(c);

        public UiHelper Helper => new() { ExtraTimeOutSeconds = 15, ShowToAll = true };
    }
    ...
    private void Run(Player Owner, GameEvent gameEvent, GameEventArgs eventArgs)
    {
        ...
        {
            Rearrangeable = [true, true],
            DefaultResult = [new List<Card>(Game.CurrentGame.Decks[GuanXingDeck]), []]
        };
        ...
        {
            Game.CurrentGame.NotificationProxy.NotifyLogEvent(new LogEvent("GuanXing", Owner, Game.CurrentGame.Decks[null, GuanXingDeck].Count, 0), [Owner], false);
            Game.CurrentGame.InsertBeforeDeal(null, Game.CurrentGame.Decks[null, GuanXingDeck], new MovementHelper() { IsFakedMove = true });
        }
        else
        {
            Game.CurrentGame.NotificationProxy.NotifyLogEvent(new LogEvent("GuanXing", Owner, answer[0].Count, answer[1].Count), [Owner], false);
