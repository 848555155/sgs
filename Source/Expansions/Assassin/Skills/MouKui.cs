...
public class MouKui : TriggerSkill
{
    ...
    private void Run1(Player Owner, GameEvent gameEvent, GameEventArgs eventArgs)
    {
        ...
        foreach (var target in eventArgs.Targets)
        {
            ...
            int answer = 0;
            Owner.AskForMultipleChoice(prompt, isNaked ? Prompt.YesNoChoices : options, out answer);
    ...
    private void Run2(Player Owner, GameEvent gameEvent, GameEventArgs eventArgs)
    {
    ...
    private static readonly CardAttribute MouKuiCheck = CardAttribute.Register("MouKuiCheck");
}
