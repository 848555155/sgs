...
{
    private class BiFaVerifier : CardsAndTargetsVerifier
    {
        public BiFaVerifier()
        {
            MaxPlayers = 1;
    ...
    private class BiFaGiveCardVerifier : CardsAndTargetsVerifier
    {
        private readonly Card theCard;
    ...
    private void Run(Player Owner, GameEvent gameEvent, GameEventArgs eventArgs, List<Card> cards, List<Player> players)
    {
    ...
    private class BiFaTrigger : Trigger
    {
        public override void Run(GameEvent gameEvent, GameEventArgs eventArgs)
        {
            ...
            if (Game.CurrentGame.Decks[Owner, BiFaDeck].Count == 1)
            {
                ...
                if (!ChenLin.IsDead && Owner.AskForCardUsage(new CardUsagePrompt("BiFaGiveCard", ChenLin), new BiFaGiveCardVerifier(theCard), out skill, out cards, out players))
                {
                    ...
                    Game.CurrentGame.NotificationProxy.NotifyLogEvent(new LogEvent("BiFa", Owner, bifa, Prompt.Fail), new List<Player>() { Owner });
                ...
                else
                {
                    ...
                    Game.CurrentGame.NotificationProxy.NotifySkillUse(log);
                    Game.CurrentGame.NotificationProxy.NotifyLogEvent(new LogEvent("BiFa", Owner, bifa, Prompt.Success), new List<Player>() { Owner });
