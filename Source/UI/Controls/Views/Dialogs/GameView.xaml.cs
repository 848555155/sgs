...
public partial class GameView : UserControl, INotificationProxy
{
    ...
    private readonly IList<StackPanel> stackPanels;
    private readonly IList<RadioButton> radioLogs;
    private readonly IList<FlowDocument> logDocs;
    private readonly ObservableCollection<PlayerView> profileBoxes;
    private readonly IDictionary<Player, PlayerViewBase> playersMap;
    private readonly GameLogs gameLogs;
    #endregion
    ...
    private Dictionary<KeyValuePair<Player, Player>, KeyValuePair<Line, Line>> _cueLines;
    private Dictionary<KeyValuePair<Player, Player>, Timeline> _lineUpAnimations;
    // Dictionary<KeyValuePair<Player, Player>, Line> _cueLineGlows;

    private static readonly int _cueLineZIndex = 100000;
    ...
    private void _CreateCueLines()
    {
        ...
        foreach (var source in playersMap.Keys)
        {
            foreach (var target in playersMap.Keys)
            {
                ...
                Storyboard.SetTargetProperty(anim1, new PropertyPath(Shape.StrokeDashOffsetProperty));
                ...
                Storyboard.SetTargetProperty(anim2, new PropertyPath(VisibilityProperty));

                var anim3 = new DoubleAnimation();
                anim3.Duration = _lineUpDuration;
                Storyboard.SetTarget(anim3, line2);
                Storyboard.SetTargetProperty(anim3, new PropertyPath(Shape.StrokeDashOffsetProperty));
                ...
                Storyboard.SetTargetProperty(anim4, new PropertyPath(VisibilityProperty));
    ...
    private void LobbyModel_OnChat(string userName, string msg)
    {
    ...
    private void playerView_OnRequestSpectate(object sender, EventArgs e)
    {
    ...
    private void mainPlayer_PropertyChanged(object sender, PropertyChangedEventArgs e)
    {
        ...
        if (e.PropertyName == "TimeOutSeconds")
        {
            ...
            progressBar.BeginAnimation(System.Windows.Controls.Primitives.RangeBase.ValueProperty, doubleanimation);
        }
    ...
    private ChildWindow _privateDeckChoiceWindow;
    ...
    private ChildWindow cardChoiceWindow;
        ...
        Application.Current.Dispatcher.Invoke((ThreadStart)delegate ()
        {
            ...
            if (e.PropertyName == "IsCardChoiceQuestionShown")
            {
                if (model.IsCardChoiceQuestionShown)
                {
                    ...
                    cardChoiceWindow.HorizontalContentAlignment = HorizontalAlignment.Center;
    ...
    private static readonly int _minHSpacing = 0;
    private static readonly int _minVSpacing = 2;
    #endregion
    ...
    private static readonly ResourceDictionary equipAnimationResources = new ResourceDictionary() { Source = new Uri("pack://application:,,,/Animations;component/EquipmentAnimations.xaml") };
    private static readonly ResourceDictionary baseCardAnimationResources = new ResourceDictionary() { Source = new Uri("pack://application:,,,/Animations;component/BaseCardAnimations.xaml") };
        ...
        if (log.SkillAction != null)
        {
            ...
            if (log.SkillAction.IsSingleUse || log.SkillAction.IsAwakening)
            {
                Application.Current.Dispatcher.BeginInvoke((ThreadStart)delegate ()
                {
                    ...
                    anim.HorizontalAlignment = HorizontalAlignment.Stretch;
                    anim.VerticalAlignment = VerticalAlignment.Stretch;
                    anim.Start();
                });
                animPlayed = true;
            }
    ...
    private void card_OnSelectedChanged(object sender, EventArgs e)
    {
