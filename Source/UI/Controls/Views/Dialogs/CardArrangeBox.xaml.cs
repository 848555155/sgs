...
public partial class CardArrangeBox : UserControl
{
    ...
    private readonly StackPanel _resultSlotPanel;
    private readonly StackPanel _resultPanel;

    public event OnCardArrange OnCardMoved;

    private void CardArrangeBox_DataContextChanged(object sender, DependencyPropertyChangedEventArgs e)
    {
        UpdateModel();
    }

    private readonly List<CardStack> _allCardStacks;

    private readonly List<CardStack> _allCardSlots;

    private readonly Dictionary<CardStack, CardChoiceLineViewModel> _stackInfo;

    private readonly Dictionary<CardView, CardStack> _originalPlace;

    private readonly Dictionary<CardStack, CardStack> _stackToSlot;
    ...
    private void _UpdateVerifiedStatus()
    {
        ...
        bool enabled = (model.Verifier.Verify(model.Answer) == VerifierResult.Success);
        ...
        foreach (var stack in _allCardStacks)
        {
            ...
            foreach (var card in stack.Cards)
            {
                ...
                foreach (var list in model.Answer)
                {
                    ...
                    list.Add(card.Card);
                    if (model.Verifier.Verify(model.Answer) != VerifierResult.Fail)
                    {
                        possible = true;
    ...
    private void cardView_OnDragging(object sender, EventArgs e)
    {
    ...
    private void cardView_OnDragBegin(object sender, EventArgs e)
    {

        InteractingCard = sender as CardView;
        InteractingCard.SetValue(Panel.ZIndexProperty, 1000);
